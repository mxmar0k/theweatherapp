...
// beacuse it adds an event listener to each city logged
// I also discovered that you can add classes here and it uses tailwind
// woooooooooow
...
// it uses the latitude and longitude parameters we obtain further in this code
...
function getWeatherDetails(latitude, longitude) {
  ...
  return fetch(url)
    ...
    .then(data => {
      ...
      //added functionality to the reverse geocoding and tailwind styles to make it look better
      //i actually dont know why it appends to the bottom, but i had to rode with it haha

      const currentWeatherHeading = document.createElement('h3');
      getCoordinates(userInput.value.trim())
        .then(coordinates => {
...
// i added this, because a friend told me when she typed 1, it actually gave coordinates, turns out 
//it is a district in viena, so i think it was better to use reverse geo, to give me the location of the city
//rather than a district, it was basically the same code we have been using all over
